...
typedef NS_ENUM(NSInteger, HMSegmentedControlSelectionStyle) {
    ...
    HMSegmentedControlSelectionStyleFixedWidth, // Indicator width is fixed
...
typedef NS_ENUM(NSInteger, HMSegmentedControlSegmentWidthStyle) {
    ...
    HMSegmentedControlSegmentWidthStyleDynamicFixed, // 1、适应字号的宽度；2、适应segement宽度
};
 ...
 */
@property (nonatomic, copy) NSDictionary *selectedTitleTextAttributes UI_APPEARANCE_SELECTOR;
...
/**
 Width of the selection indicator. Only effective when `HMSegmentedControlSelectionStyle` is `HMSegmentedControlSelectionStyleFixedWidth`.
...
/**
 是否绘制右上角角标， 默认 NO, 打开绘制默认绘制索引0
 */
@property (nonatomic, assign, getter=isDrawBadge) BOOL drawBadge;
...
/**
 可能展示红点的tab
 */
@property (nonatomic, strong) CALayer *redBadgeTabLayer;

/**
 默认为NO, 重复点击相同index,会触发index回调
 */
@property (nonatomic, assign) BOOL  allowRepeatTap;
