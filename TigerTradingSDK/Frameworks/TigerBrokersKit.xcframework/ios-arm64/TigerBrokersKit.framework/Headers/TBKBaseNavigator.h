...
#import "TigerBrokersKit/TBKNavigatorPersistenceMode.h"

@protocol TBKNavigatorDelegate;
@protocol TBKNavigatorRootContainer;
@class TBKURLAction;
@class TBKURLMap;
@class TBKURLPattern;
 ...
 */
@interface TBKBaseNavigator : NSObject <UIPopoverControllerDelegate> {
    TBKURLMap*                   _URLMap;                // 只读属性
    
//    UIViewController*           _rootViewController;    // 只读属性
    NSMutableArray*             _delayedControllers;    // 无属性
    id                          _popoverController;     // 无属性
    
    BOOL                        _delayCount;            // 无属性
}
 ...
 * @see TBKURLMap
 */
@property (nonatomic, readonly) TBKURLMap* URLMap;
 ...
 * @default nil
 */
@property (nonatomic, weak) id<TBKNavigatorRootContainer> rootContainer;
 ...
 *
 * @default TBKNavigatorPersistenceModeNone
 */
@property (nonatomic) TBKNavigatorPersistenceMode persistenceMode;
...
@property (nonatomic, weak) id<TBKNavigatorDelegate> delegate;
 ...
 * conforms to the TBKNavigatorRootContainer protocol, we call getNavigatorForController:
 ...
 * returns the global navigator via [TBKBaseNavigator globalNavigator].
 *
 ...
 */
+ (TBKBaseNavigator*)navigatorForView:(UIView*)view;

+ (TBKBaseNavigator*)globalNavigator;
+ (void)setGlobalNavigator:(TBKBaseNavigator*)navigator;

/**
 * Load and display the view controller with a pattern that matches the URL.
 *
 * This method replaces all other openURL methods by using the chainable TBKURLAction object.
 *
 ...
 *
 * Example TBKURLAction initialization:
 * [[TBKURLAction actionWithURLPath:@"tt://some/path"]
 *                   applyAnimated:YES]
 *
 * Each apply* method on the TBKURLAction object returns self, allowing you to chain methods
 ...
 */
- (UIViewController*)openURLAction:(TBKURLAction*)URLAction;
