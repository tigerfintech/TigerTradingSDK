...
@interface TBKURLRequestQueue : NSObject {
@private
...
/**
 * The default Timeout used for all TBKURLRequests.
 * 
 * This timeout is applied to all requests that have a negative timeout set.
 *
 * The default value is defined as kTimeout in TBKURLRequestQueue.m
 *
 * @see TBKURLRequest::timeoutInterval
 */
 ...
 */
+ (TBKURLRequestQueue *)mainQueue;

/**
 * Set the shared cache singleton used across the application.
 */
+ (void)setMainQueue:(TBKURLRequestQueue *)queue;
 ...
 */
- (BOOL)sendRequest:(TBKURLRequest *)request;
 ...
 */
- (BOOL)sendSynchronousRequest:(TBKURLRequest *)request;

/**
 * Cancel a request that is in progress.
 */
- (void)cancelRequest:(TBKURLRequest *)request;
...
/**
 * Create a Cocoa URL request from a TigerBrokersKit URL request.
 */
- (NSURLRequest*)createNSURLRequest:(TBKURLRequest *)request URL:(NSURL*)URL;
