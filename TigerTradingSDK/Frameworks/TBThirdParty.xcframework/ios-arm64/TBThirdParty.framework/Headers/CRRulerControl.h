 ...
 */
IB_DESIGNABLE
@interface CRRulerControl : UIControl <CRRulerLayerMarkDelegate, UIScrollViewDelegate>
 ...
 */
@property (nonatomic, readonly) UIImageView *pointerImageView; // 指针图像

@property (nonatomic, strong) UILabel *pointerValueLabel; // 显示指针值的标签

/**
 *  Spacing between center of nearby marks
 */
@property (nonatomic) IBInspectable CGFloat spacingBetweenMarks; // 刻度间距

/// 刻度的左右边距. 刻度的左右各有一段距离可以自定义绘制.
@property (nonatomic, assign) IBInspectable CGFloat sideOffset;

/**
 *  All width of rulerControl default 1000
 */
@property (nonatomic, readonly) IBInspectable CGFloat rulerWidth; // ruler总长度. 由 spacingBetweenMarks * rangeLength + 2 * sideOffset 被动确定
/**
 *  Text value of left mark on rulerControl default - 10
 */
@property (nonatomic) IBInspectable CGFloat rangeFrom; // 刻度起始值
/**
 *  Length rulerControl beginning from rangeFrom default 20
 */
@property (nonatomic) IBInspectable CGFloat rangeLength; // 刻度数量
/**
 *  Value on center of CRRulerControl. Thay variable by proterty rangeFrom and rangeLength default 0
 */
@property (nonatomic) IBInspectable CGFloat value; // 刻度值
